...
from models.replay_buffer import ReplayBuffer, PrioritizedReplayBuffer, Transition
import os
from torch.utils.tensorboard import SummaryWriter
import random
import matplotlib.pyplot as plt

def train_dqn(env_name='ALE/AirRaid-v5', num_episodes=30000, batch_size=128, gamma=0.994,
              ...
              use_per=False, per_alpha=0.6, per_beta=0.4, per_eps=1e-6):
    ...
    target_net.eval()
    if use_per:
        memory = PrioritizedReplayBuffer(replay_capacity, human_ratio=human_ratio, alpha=per_alpha, beta=per_beta, eps=per_eps)
        print(f"Using Prioritized Replay Buffer: alpha={per_alpha}, beta={per_beta}, eps={per_eps}")
    else:
    ...
    for episode in range(start_episode, start_episode + num_episodes):
        ...
        while not done:
            ...
            if len(memory) >= batch_size:
                sample_result = memory.sample(batch_size, n_step, gamma)
                # Handle PER return (batch, indices, weights) vs uniform list
                if use_per:
                    transitions, indices, is_weights = sample_result
                    is_weights = torch.tensor(is_weights, device=device)
                else:
                ...
                if is_weights is not None:
                    per_sample_loss = F.smooth_l1_loss(state_action_values, expected_state_action_values.unsqueeze(1), reduction='none').squeeze()
                ...
                # Update PER priorities using absolute TD errors
                if use_per:
                    with torch.no_grad():
                        td_errors = (expected_state_action_values.unsqueeze(1) - state_action_values).squeeze().abs().detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--per_alpha', type=float, default=0.6, help='PER alpha (priority exponent)')
    parser.add_argument('--per_beta', type=float, default=0.4, help='PER beta (IS weight exponent)')
    parser.add_argument('--per_eps', type=float, default=1e-6, help='PER epsilon added to TD error')
