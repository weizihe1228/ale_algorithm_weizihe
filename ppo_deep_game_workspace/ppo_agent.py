...
class CNNPolicy(nn.Module):
    def __init__(self, in_channels: int, num_actions: int, input_size: int = 84):
        super().__init__()
        self.features = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            # Added capacity: two extra conv layers
            nn.Conv2d(64, 128, kernel_size=3, stride=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1),
            nn.ReLU(inplace=True),
        )
        # Compute flattened size for configurable input
        with torch.no_grad():
...
class PPOAgent:
    def __init__(self, in_channels: int, num_actions: int, config: PPOConfig, input_size: int = 84):
        self.config = config
        self.device = torch.device(config.device)
        self.policy = CNNPolicy(in_channels, num_actions, input_size=input_size).to(self.device)
