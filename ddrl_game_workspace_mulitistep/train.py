...
def train_dqn(env_name='ALE/AirRaid-v5', num_episodes=10000, batch_size=128, gamma=0.994,
              ...
              log_path='logs/', checkpoint_path=None, start_episode=0, human_demo_path=None, human_ratio=0.5, n_step=1):
        ...
        while not done:
            ...
            if len(memory) >= batch_size:
                ...
                if len(non_final_next_states_list) > 0:
                    ...
                    with torch.no_grad():
                        # DDQN: Use policy_net to select best action, target_net to evaluate
                        best_actions = policy_net(non_final_next_states).max(1)[1]
                        next_state_values[non_final_mask] = target_net(non_final_next_states).gather(1, best_actions.unsqueeze(1)).squeeze(1)
